 ...
 * Run from repository root: node dist/benchmark/benchmark.js
...
function run(name: string, fn: (i: number) => void, iterations: number): { name: string; opsPerSec: number; avgMs: number; elapsed: number } {
  const start = ns();
